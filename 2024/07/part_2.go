...
func concatNumbers(num1 int, num2 int) int {
	concatStr := strconv.Itoa(num1) + strconv.Itoa(num2)
		...
		for i := 0; i < len(permutations); i++ {
			...
			for j := 0; j < len(permutations[i]); j++ {
				...
				if j == 0 {
					...
					} else if permutations[i][j] == "*" {
						sum += numbers[j] * numbers[j+1]
					} else {
						sum = concatNumbers(numbers[j], numbers[j+1])
					}
				} else {
					if permutations[i][j] == "+" {
						sum += numbers[j+1]
						...
						sum = concatNumbers(sum, numbers[j+1])
					}
				}
			}
