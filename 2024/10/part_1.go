...
func main() {
	...
	for key, group := range grouped {
		...
		distinctEndings := make(map[string][][][2]int)

		for _, route := range group {

			fmt.Println(route)

			// Get distinct route endings
			key := fmt.Sprintf("%d", route[len(route)-1])

			// Add the route to the group
			distinctEndings[key] = append(distinctEndings[key], route)
		}
		sumOfScores += len(distinctEndings)
	}
